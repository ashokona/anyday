...
var JobSeekerSearchComponent = (function () {
    ...
    JobSeekerSearchComponent.prototype.initializeFilterJobseeker = function () {
        ...
        var jobseekersFilter = {
            Date: __WEBPACK_IMPORTED_MODULE_9_moment__(date).format('LL'),
            Hours_Guaranteed: null,
            Position: '',
            pay_request: null,
            distance: null
    ...
    JobSeekerSearchComponent.prototype.ngOnInit = function () {
        ...
        this.userService.getPositions().subscribe(function (res) {
            _this.positionList = res.data;
        });
    };
    // constant for filter jobseeker
    JobSeekerSearchComponent.prototype.FilterJobseeker = function () {
        var jobseekersFilter = {
            Date: '',
            Hours_Guaranteed: null,
            Position: '',
        ...
        jobseekersFilter.Date = __WEBPACK_IMPORTED_MODULE_9_moment__(this.filterJobseekers.Date).format('LL');
    ...
    JobSeekerSearchComponent.prototype.getJobseekers = function (data) {
        ...
        this.employerService.queryJobseekers(data).subscribe(function (res) {
            ...
            _this.jobseekers = [];
            _this.calculateDistance(res.data);
            _this.loaderService.display(false);
        }, function (err) {
            _this.loaderService.display(false);
            _this.notificationsService.error(err.title, err.error.message, __WEBPACK_IMPORTED_MODULE_7__environments_environment__["a" /* environment */].options);
        ...
        jobseekers.forEach(function (jobseeker) {
            if (jobseeker.JS_id.locationLat && jobseeker.JS_id.locationLng) {
                ...
                if (jobseeker.Distance <= jobseeker.JS_id.Travel_Distance) {
                    if (_this.jobseekers.length > 0) {
                        var count = 0;
                        for (var i = 0; i < _this.jobseekers.length; i++) {
                            if (_this.jobseekers[i]._id == jobseeker._id) {
                                count++;
                            }
                        }
                        if (count == 0) {
                            _this.jobseekers.push(jobseeker);
                        }
                    }
                    else {
                        _this.jobseekers.push(jobseeker);
...
var PaymentComponent = (function () {
    function PaymentComponent(jsonLoaderService, employerservice, userService, loaderService, notificationsService, route) {
        ...
        if (this.employerservice.itemsToHire.length > 0) {
            this.employerservice.verifyPayment({ offerDate: this.employerservice.itemsToHire[0].Date }).subscribe(function (res) {
                if (!res.data.paymentRequired) {
                    _this.initializePayment();
                    _this.releaseOffer(res.data.paymentid);
                }
                else {
                }
            });
    ...
    PaymentComponent.prototype.makePayment = function () {
        ...
        this.employerservice.makePayment(this.payment).subscribe(function (res) {
            if (res.message == 'Payment Sucessfull') {
                _this.initializePayment();
                _this.releaseOffer(res.data._id);
            }
        }, function (err) {
            _this.loaderService.display(false);
    ...
    PaymentComponent.prototype.releaseOffer = function (paymentId) {
        var _this = this;
        this.employerservice.postOffer(paymentId).subscribe(function (res) {
            _this.loaderService.display(false);
...
var PriorJobOfferComponent = (function () {
    function PriorJobOfferComponent(employerService, notificationsService, loaderService, paginationService) {
        var _this = this;
        this.employerService = employerService;
        this.notificationsService = notificationsService;
        this.loaderService = loaderService;
    ...
    PriorJobOfferComponent.prototype.setPage = function (page) {
...
var TransactionHistoryComponent = (function () {
    function TransactionHistoryComponent(employerService, notificationsService, loaderService, paginationService) {
        var _this = this;
        this.employerService = employerService;
        this.notificationsService = notificationsService;
        this.loaderService = loaderService;
    ...
    TransactionHistoryComponent.prototype.setPage = function (page) {
        if (page < 1 || page > this.pager.totalPages) {
            return;
        }
        // get pager object from service
...
var routes = [
    {
        ...
        children: [
            { path: '', component: __WEBPACK_IMPORTED_MODULE_7__components_profile_profile_component__["a" /* ProfileComponent */], pathMatch: 'full' },
            { path: 'search', component: __WEBPACK_IMPORTED_MODULE_3__components_job_seeker_search_job_seeker_search_component__["a" /* JobSeekerSearchComponent */], pathMatch: 'full' },
            // {path:'profile', component:ViewProfileComponent},
            { path: 'transactionhistory', component: __WEBPACK_IMPORTED_MODULE_4__components_transaction_history_transaction_history_component__["a" /* TransactionHistoryComponent */], pathMatch: 'full' },
            { path: 'joboffers', component: __WEBPACK_IMPORTED_MODULE_5__components_prior_job_offer_prior_job_offer_component__["a" /* PriorJobOfferComponent */], pathMatch: 'full' },
            { path: 'payment', component: __WEBPACK_IMPORTED_MODULE_6__components_payment_payment_component__["a" /* PaymentComponent */], pathMatch: 'full' },
            { path: 'profile', component: __WEBPACK_IMPORTED_MODULE_7__components_profile_profile_component__["a" /* ProfileComponent */], pathMatch: 'full' },
            { path: 'changepassword', component: __WEBPACK_IMPORTED_MODULE_8__shared_components_change_password_change_password_component__["a" /* ChangePasswordComponent */], pathMatch: 'full' }
        ]
    }
];
...
var EmployerService = (function () {
    ...
    //post offer method
    EmployerService.prototype.postOffer = function (paymentId) {
    ...
    EmployerService.prototype.verifyPayment = function (data) {
        var route = 'payments/verifypayment';
        return this.apiService.post(route, data).map(function (res) {
            return res;
        });
    };
